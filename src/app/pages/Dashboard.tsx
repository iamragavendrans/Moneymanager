...
import {
  ArrowUpRight, ArrowDownRight, Eye, EyeOff, LayoutGrid, ChevronDown,
  TrendingUp, X, Check, Clock, ChevronLeft, ChevronRight,
} from "lucide-react";
import {
  format, subDays, getDaysInMonth, getDay, startOfMonth, endOfMonth,
  startOfWeek, endOfWeek, subWeeks, eachDayOfInterval, addWeeks,
  subMonths, addMonths, subYears, addYears, isSameDay, isSameMonth,
  endOfDay, startOfYear, endOfYear,
} from "date-fns";
...
const Card = ({ children, className = "", onClick }: CardProps) => (
  <div
    onClick={onClick}
    className={`bg-white rounded-2xl p-4 md:p-5 shadow-sm border border-slate-100 ${className}`}
  >
    {children}
  </div>
);
...
interface ListCardProps {
  ...
  amount: string;
  badgeText?: string;
  badgeType?: "danger" | "success" | "warning" | "default";
}
const ListCard = ({ icon, title, subtitle, amount, badgeText, badgeType }: ListCardProps) => (
...
type ItemStatus = "pending" | "paid" | "hold";
...
const SwipeableCard = ({
  ...
  rightActionLabel, leftActionLabel, rightActionIcon, leftActionIcon, onClick,
}: SwipeableCardProps) => {
    ...
    <div className="relative overflow-hidden rounded-xl bg-slate-100 cursor-pointer mb-1 select-none">
      <div className="absolute inset-0 flex justify-between items-center px-5">
        ...
        <div className="text-red-500 font-bold text-xs flex items-center gap-1">
          {leftActionLabel} {leftActionIcon}
        </div>
      </div>
      <div
...
export const Dashboard = () => {
  const { getNetWorth, transactions, accounts, profile, updateProfile, investments, entities } =
    useFinance();
  ...
  // Per-session swipe status for upcoming / scheduled items (resets on refresh by design)
  const [itemStatuses, setItemStatuses] = useState<Record<string, ItemStatus>>({});
  const getStatus = (id: string): ItemStatus => itemStatuses[id] ?? "pending";
  ...
  const navigatePeriod = (dir: "prev" | "next") => {
    ...
    if (globalFilter === "1W")
      newDate = dir === "prev" ? subWeeks(referenceDate, 1) : addWeeks(referenceDate, 1);
    if (globalFilter === "1M")
      newDate = dir === "prev" ? subMonths(referenceDate, 1) : addMonths(referenceDate, 1);
    if (globalFilter === "1Y")
      newDate = dir === "prev" ? subYears(referenceDate, 1) : addYears(referenceDate, 1);
  ...
  const historicalAccounts = useMemo(() => {
    ...
    return accounts.map((a: Account) => {
      const futureTx = transactions.filter(
        (t) => new Date(t.date) > endOfDay(referenceDate) && (t.account_id === a.id || t.to_account_id === a.id)
      );
      let netChange = 0;
      futureTx.forEach((t) => {
  ...
  const historicalNetWorth = useMemo(
    () => historicalAccounts.reduce((s, a) => s + a.balance, 0) + totalInvestmentValue,
    [historicalAccounts, totalInvestmentValue]
  );
  const historicalAssets = useMemo(
    () =>
      historicalAccounts
        .filter((a) => a.type !== "credit_card" && a.type !== "loan")
        .reduce((s, a) => s + a.balance, 0) + totalInvestmentValue,
    [historicalAccounts, totalInvestmentValue]
  );
  const historicalLiabilities = useMemo(
    () =>
      historicalAccounts
        .filter((a) => a.type === "credit_card" || a.type === "loan")
  ...
  const prevPeriodStats = useMemo(() => {
    const days = globalFilter === "1M" ? 30 : globalFilter === "1Y" ? 365 : 7;
    ...
    const prevAccounts = accounts.map((a) => {
      const futureTx = transactions.filter(
        (t) => new Date(t.date) > endOfDay(currentStart) && (t.account_id === a.id || t.to_account_id === a.id)
      );
      let netChange = 0;
      futureTx.forEach((t) => {
        if (t.type === "income" && t.account_id === a.id) netChange += t.amount;
    ...
    const pNW = prevAccounts.reduce((s, a) => s + a.balance, 0);
  ...
  const chartData = useMemo(() => {
    const loopLimit = globalFilter === "1Y" ? 30 : globalFilter === "1M" ? 30 : 7;
    const data = [];
  ...
  // Derive upcoming payments from real entities (subscriptions + recurring)
  const upcomingItems = useMemo(() => {
    return entities
      .filter(
        (e) =>
          (e.type === "subscription" || e.type === "recurring") &&
          typeof e.amount === "number" &&
          e.amount > 0
      )
      .map((e) => {
        let dueLabel = e.frequency ?? "Recurring";
        let badgeType: ListCardProps["badgeType"] = "default";
        if (e.nextDue) {
          const days = Math.ceil(
            (new Date(e.nextDue).getTime() - Date.now()) / 86400000
          );
          if (days < 0) { dueLabel = "Overdue"; badgeType = "danger"; }
          else if (days === 0) { dueLabel = "Due Today"; badgeType = "danger"; }
          else if (days <= 3) { dueLabel = `Due in ${days}d`; badgeType = "danger"; }
          else if (days <= 7) { dueLabel = `Due in ${days}d`; badgeType = "warning"; }
          else { dueLabel = `Due ${format(new Date(e.nextDue), "MMM dd")}`; badgeType = "default"; }
        }
        return {
          id: e.id,
          title: e.name,
      ...
      .sort((a, b) => {
        const order: Record<string, number> = { danger: 0, warning: 1, default: 2 };
        return (order[a.type ?? "default"] ?? 2) - (order[b.type ?? "default"] ?? 2);
      })
      .slice(0, 5);
  }, [entities]);
  ...
  const renderHeatmap = () => {
    ...
    if (globalFilter === "1M" || globalFilter === "1W") {
      ...
      const refStart =
        globalFilter === "1W" ? startOfWeek(referenceDate) : startOfMonth(referenceDate);
      const refEnd =
        globalFilter === "1W" ? endOfWeek(referenceDate) : endOfMonth(referenceDate);
      ...
      const matrix = Array(matrixRows)
        .fill(null)
        .map(() => Array<Date | null>(7).fill(null));
      days.forEach((d, i) => {
        matrix[Math.floor(i / 7)][getDay(d)] = d;
      });
      return (
        <div className="flex flex-col gap-2 w-full overflow-x-auto pb-4 scrollbar-hide">
          <div className="flex gap-2 min-w-max pl-8">
            {["S", "M", "T", "W", "T", "F", "S"].map((day, i) => (
              <div key={i} className="w-6 text-center text-[10px] font-bold text-slate-400">{day}</div>
            ))}
          </div>
                ...
                {week.map((day, dIdx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-6 h-6 rounded-[4px] ${getBgColor(stats)} hover:scale-110 hover:ring-2 ring-slate-300 cursor-pointer transition-all`}
                      title={`${format(day, "MMM dd")}: ${stats.type}`}
                    />
                  );
                })}
              </div>
      ...
      <div className="flex flex-col gap-1.5 w-full overflow-x-auto pb-4 scrollbar-hide">
        <div className="flex gap-1.5 min-w-max pl-8">
          {Array.from({ length: 31 }).map((_, i) => (
            <div key={i} className="w-4 text-center text-[8px] font-bold text-slate-400">{i + 1}</div>
          ))}
        </div>
        {Array.from({ length: 12 }).map((_, mIdx) => {
          ...
          return (
            <div key={mIdx} className="flex gap-1.5 min-w-max items-center">
              <div className="w-6 text-right text-[10px] font-bold text-slate-400 pr-1">
                {monthNames[mIdx]}
              </div>
              {Array.from({ length: 31 }).map((_, dIdx) => {
                if (dIdx >= daysInM)
                  return <div key={dIdx} className="w-4 h-4 bg-transparent pointer-events-none" />;
                ...
                return (
                  <div
                    ...
                    className={`w-4 h-4 rounded-[3px] ${getBgColor(stats)} hover:scale-125 cursor-pointer transition-all`}
                    title={`${monthNames[mIdx]} ${dIdx + 1}: ${stats.type}`}
                  />
                );
              })}
            </div>
  ...
  return (
    <div className="max-w-[1200px] mx-auto w-full relative">
      ...
      <div className="sticky top-0 z-40 bg-slate-50/80 backdrop-blur-md px-4 py-3 -mx-4 border-b border-slate-200/50 mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg p-1 shadow-sm">
          ...
          <span className="text-slate-900 text-[11px] font-bold w-24 text-center">
            {isSameMonth(referenceDate, new Date()) && isSameDay(referenceDate, new Date())
              ? "TODAY"
              : format(referenceDate, globalFilter === "1Y" ? "yyyy" : "MMM dd, yyyy")}
          </span>
          <button
      ...
      <div className="px-4 pb-10 space-y-6">
            ...
            return (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
                <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 max-h-[80vh] flex flex-col">
                  ...
                  <div className="overflow-y-auto flex-1 space-y-2">
                    {dayTxs.length === 0 ? (
                      ...
                      dayTxs.map((tx) => (
                        <div
                          key={tx.id}
                          onClick={() => { setSelectedDate(null); setEditTxId(tx.id); }}
                          className="flex items-center justify-between p-3 bg-slate-50 rounded-xl hover:bg-indigo-50 cursor-pointer transition-colors"
                        >
                          <div className="flex items-center gap-3">
                            <div
                  ...
                  <button
                    onClick={() => setSelectedDate(null)}
                    className="mt-4 w-full py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold transition-all"
                  >
                    Close
                  </button>
                </div>
        ...
        {/* Hero Net Worth Card */}
        <div className="bg-[#0B1220] text-white rounded-[24px] p-6 md:p-8 shadow-xl relative overflow-hidden w-full min-h-[220px] flex flex-col justify-between">
          <div className="absolute inset-x-0 bottom-0 h-44 opacity-60 z-0 pointer-events-none">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                <Line type="monotone" dataKey="cumulativeNW" stroke="#10B981" strokeWidth={4} dot={false} isAnimationActive />
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            ...
            <div className="mt-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                <h1 className="text-[40px] md:text-[48px] font-bold tracking-tight mb-1">
                  {isMasked ? "₹ •••••••" : formatINR(historicalNetWorth)}
                </h1>
                <div className="flex items-center gap-2">
                  {netWorthChange >= 0 ? (
                  ...
                  >
                    {Math.abs(netWorthChange).toFixed(1)}% vs last{" "}
                    {periodTitle.toLowerCase().replace("ly", "")}
                  </span>
                </div>
              </div>
              <button
                onClick={() => setShowHeroBreakdown(!showHeroBreakdown)}
                className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-sm font-semibold transition-colors backdrop-blur-md border border-white/10 w-fit active:scale-95"
              >
                Breakdown{" "}
                <ChevronDown
                  className={`w-4 h-4 transition-transform ${
                    showHeroBreakdown ? "rotate-180" : ""
                  }`}
                />
              </button>
            </div>
          ...
          <div
            className={`relative z-10 w-full overflow-hidden transition-all duration-300 ease-in-out ${
              showHeroBreakdown ? "max-h-40 mt-6 opacity-100" : "max-h-0 opacity-0"
            }`}
          >
            <div className="p-4 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md grid grid-cols-2 gap-4">
              <div>
                <p className="text-slate-400 text-[10px] md:text-xs mb-1 uppercase tracking-wider font-semibold">
                  Total Assets
                </p>
                <p className="font-bold text-lg md:text-xl">
                  {isMasked ? "₹ •••••" : formatINR(historicalAssets)}
                </p>
              </div>
              <div>
                <p className="text-slate-400 text-[10px] md:text-xs mb-1 uppercase tracking-wider font-semibold">
                ...
                <p className="font-bold text-lg md:text-xl text-red-400">
                  {isMasked ? "₹ •••••" : formatINR(historicalLiabilities)}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 w-full">
          <Card
            onClick={() =>
              navigate(
                `/transactions?type=income&start=${subDays(
                  now,
                  globalFilter === "1W" ? 7 : globalFilter === "1M" ? 30 : 365
                ).getTime()}&end=${endOfDay(now).getTime()}`
              )
            }
            className="flex flex-col justify-between relative overflow-hidden group hover:shadow-md transition-all h-[140px] md:h-[160px] cursor-pointer"
          >
            <div className="flex justify-between items-start mb-2">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-500 shrink-0 border border-emerald-100">
            ...
            <h3 className="text-xl md:text-3xl font-bold text-slate-800 truncate mb-1">
              {isMasked ? "₹ •••••" : formatINR(periodIncome)}
            </h3>
            <div className="flex justify-between items-end">
              <div
                className={`flex items-center gap-1 text-[10px] md:text-xs font-medium px-1.5 md:px-2 py-0.5 md:py-1 rounded-md ${
                  incomeChange >= 0
              ...
              >
                ...
                <span className="hidden lg:inline">
                  vs last {periodTitle.toLowerCase().replace("ly", "")}
                </span>
              </div>
              <div className="flex items-end gap-1 h-8 opacity-80">
                {barsIncome.map((h, i) => (
                  <div
                    key={i}
                    className="w-1 md:w-1.5 bg-emerald-200 rounded-t-sm"
                    style={{ height: `${h}%` }}
                  />
                ))}
              </div>
          ...
          <Card
            onClick={() =>
              navigate(
                `/transactions?type=expense&start=${subDays(
                  now,
                  globalFilter === "1W" ? 7 : globalFilter === "1M" ? 30 : 365
                ).getTime()}&end=${endOfDay(now).getTime()}`
              )
          ...
          >
            ...
            <h3 className="text-xl md:text-3xl font-bold text-slate-800 truncate mb-1">
              {isMasked ? "₹ •••••" : formatINR(periodExpense)}
            </h3>
            <div className="flex justify-between items-end">
              <div
                className={`flex items-center gap-1 text-[10px] md:text-xs font-medium px-1.5 md:px-2 py-0.5 md:py-1 rounded-md ${
              ...
              <div className="flex items-end gap-1 h-8 opacity-80">
                {barsExpense.map((h, i) => (
                  <div
                    key={i}
                    className="w-1 md:w-1.5 bg-red-300 rounded-t-sm"
                    style={{ height: `${h}%` }}
                  />
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-7 space-y-6">
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-slate-800 text-[16px]">
                    Cashflow Intensity • {periodLabel}
                  </h3>
                  <button
                    onClick={() => setShowHeatmap(!showHeatmap)}
                    className="text-sm border rounded-lg px-3 py-1.5 font-medium flex items-center gap-2 transition-colors bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
                  >
                    {showHeatmap ? <TrendingUp className="w-4 h-4" /> : <LayoutGrid className="w-4 h-4" />}
                    <span className="hidden sm:inline">{showHeatmap ? "Trendline" : "Heatmap"}</span>
                  </button>
                </div>
                <div className="mt-3 flex items-center gap-4">
                  {!showHeatmap ? (
                    <>
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500">
                        <div className="w-2 h-2 rounded-full bg-blue-500" /> Net
                      </div>
                    </>
                  ) : (
                    <>
                      <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500">
                        <div className="w-2 h-2 rounded-sm bg-emerald-400" /> Income
                      </div>
                      <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500">
                        <div className="w-2 h-2 rounded-sm bg-red-400" /> Expense
                      </div>
                      <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500">
                        <div className="w-2 h-2 rounded-sm bg-blue-400" /> Transfer
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="h-[250px] w-full animate-in fade-in duration-300">
                  <ResponsiveContainer width="100%" height={250}>
                    <LineChart data={chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                      ...
                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "#64748B", fontWeight: 500 }} tickFormatter={(v) => `₹${v / 1000}k`} tickCount={4} />
                      <Tooltip
            ...
            <Card className="p-5 hidden lg:block">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to="/transactions" className="text-indigo-600 text-sm font-semibold hover:underline">
                  View All
                </Link>
              </div>
              <div className="space-y-1">
                {recentTransactions.length === 0 ? (
                  <p className="text-sm text-slate-400 text-center py-6">No transactions yet. Add your first one!</p>
                ) : (
                  recentTransactions.map((tx) => (
                    <div key={tx.id} onClick={() => setEditTxId(tx.id)} className="cursor-pointer">
                      <ListCard
                        ...
                        title={tx.payee}
                        subtitle={`${tx.category} • ${format(new Date(tx.date), "MMM dd")}`}
          ...
          <div className="lg:col-span-5 space-y-6">
            {/* Upcoming Payments (real entity data) */}
            <Card className="p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-[15px] text-slate-800">
                  Upcoming Payments
                  <span className="text-[10px] text-slate-400 font-normal ml-2 hidden sm:inline">(Swipe LR)</span>
                </h3>
                <Link to="/settings" className="text-indigo-600 text-sm font-semibold hover:underline">
                  Manage
                </Link>
              </div>
              <div className="space-y-1">
                {upcomingItems.length === 0 ? (
                  <p className="text-sm text-slate-400 text-center py-4">
                    ...
                    <Link to="/settings" className="text-indigo-600 font-semibold">Add in Settings</Link>
                  </p>
                ) : (
                  upcomingItems.map((item) => {
                    ...
                    return (
                      <SwipeableCard
                        key={item.id}
                        rightActionLabel={status === "paid" ? "Undo" : "Paid"}
                        rightActionIcon={<Check className="w-4 h-4" />}
                        leftActionLabel={status === "hold" ? "Undo" : "Hold"}
                        leftActionIcon={<Clock className="w-4 h-4" />}
                        onSwipeRight={() =>
                          setStatus(item.id, status === "paid" ? "pending" : "paid")
                        }
                        onSwipeLeft={() =>
                          setStatus(item.id, status === "hold" ? "pending" : "hold")
                        }
                      >
                        ...
                        subtitle={
                          <span className="flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] font-semibold bg-slate-100 px-1.5 py-0.5 rounded text-slate-500">
                              {tx.type === "transfer" && tx.to_account_id
        ...
        {editTxId && (
          <TransactionFormModal txId={editTxId} onClose={() => setEditTxId(null)} />
        )}
      </div>
    </div>
  );
