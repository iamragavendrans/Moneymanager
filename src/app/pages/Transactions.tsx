...
import {
  ...
  startOfYear, endOfYear, subDays, subMonths, subYears, parse, endOfDay,
} from "date-fns";
import { useSearchParams } from "react-router";
import {
  Search, Filter, ArrowUpRight, ArrowDownRight, Wallet, ChevronDown, ChevronRight,
  Maximize2, Minimize2, Trash2, Edit, X, CheckSquare, Square, CalendarDays,
  CreditCard, Tag, Store, CheckCheck, AlertTriangle,
} from "lucide-react";
...
export const Transactions = () => {
  ...
  const [filterType, setFilterType]     = useState<"all" | "expense" | "income" | "transfer">(
    (searchParams.get("type") as "all" | "expense" | "income" | "transfer") ?? "all"
  );
  ...
  const handleDateFilter = (type: string) => {
    ...
    if (type === "last_week")   { start = startOfWeek(subDays(now,7)); end = endOfWeek(subDays(now,7)); }
    if (type === "this_month")  { start = startOfMonth(now);           end = endOfMonth(now); }
    if (type === "last_month")  { start = startOfMonth(subMonths(now,1)); end = endOfMonth(subMonths(now,1)); }
    if (type === "this_year")   { start = startOfYear(now);            end = endOfYear(now); }
    if (type === "last_year")   { start = startOfYear(subYears(now,1)); end = endOfYear(subYears(now,1)); }
    if (!start || !end) {
      searchParams.delete("start"); searchParams.delete("end");
  ...
  const groupedTransactions = useMemo(() => {
    ...
    filteredTransactions.forEach((tx) => {
      ...
      if (tx.type === "transfer") group.totalTransfer += tx.amount;

      const payeeKey = tx.payee.toLowerCase().trim();
      if (!group.payeeGroups.has(payeeKey)) {
        group.payeeGroups.set(payeeKey, { payee: tx.payee, type: tx.type, total: 0, latestDate: 0, transactions: [] });
      }
      const pg = group.payeeGroups.get(payeeKey)!;
      pg.total += tx.type === pg.type ? tx.amount : -tx.amount;
      const ts = Number(tx.id.split("_")[1]) || 0;
      if (ts > pg.latestDate) { pg.latestDate = ts; pg.payee = tx.payee; }
      pg.transactions.push(tx);
    });

    return Array.from(groups.values())
      .map((g) => ({
        ...g,
        payeeGroups: Array.from(g.payeeGroups.values()).sort((a, b) => b.latestDate - a.latestDate),
      }))
  ...
  const actualGroupCount = useMemo(() => {
    ...
    groupedTransactions.forEach((g) =>
      g.payeeGroups.forEach((pg) => { if (pg.transactions.length > 1) count++; })
    );
    return count;
  }, [groupedTransactions]);
  ...
  const toggleAll = () => {
    ...
    groupedTransactions.forEach((g) =>
      g.payeeGroups.forEach((pg) => {
        if (pg.transactions.length > 1) allIds.add(`${g.date}-${pg.payee.toLowerCase()}`);
      })
    );
    if (expandedGroups.size >= allIds.size && allIds.size > 0) {
      setExpandedGroups(new Set()); setIsAllExpanded(false);
    } else {
      setExpandedGroups(allIds); setIsAllExpanded(true);
    }
  };
  ...
  const selectAllFiltered = () => setBulkSelected(new Set(filteredTransactions.map((t) => t.id)));
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto flex flex-col h-full">
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-xs shadow-2xl text-center space-y-4">
            <div className="w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center mx-auto">
              <AlertTriangle className="w-6 h-6 text-red-500" />
            </div>
            <div>
              <p className="font-bold text-slate-800">Delete transaction?</p>
              <p className="text-sm text-slate-500 mt-1">This will reverse the account balance. This cannot be undone.</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-6 shadow-2xl animate-in slide-in-from-bottom-full duration-300">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowBulkEdit(false)} className="p-2 rounded-full hover:bg-slate-100">
                <X className="w-5 h-5 text-slate-500" />
              </button>
            </div>
            <p className="text-xs text-slate-400 mb-5">Only filled fields will be applied.</p>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Change Payee</label>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={applyBulkEdit} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 flex items-center justify-center gap-2">
                <CheckCheck className="w-4 h-4" /> Apply to {bulkSelected.size}
              </button>
            </div>
          </div>
        </div>
      ...
      {startParam && endParam && (
        <div className="bg-indigo-50 border border-indigo-100 p-3 rounded-xl mb-4 flex items-center justify-between">
          <span className="text-sm font-medium text-indigo-800">
            Filtering from <b>{format(new Date(parseInt(startParam)), "MMM dd, yyyy")}</b> to{" "}
            <b>{format(new Date(parseInt(endParam)), "MMM dd, yyyy")}</b>
          </span>
          <button
            onClick={() => { searchParams.delete("start"); searchParams.delete("end"); setSearchParams(searchParams); setCustomRange(false); }}
            className="text-indigo-600 hover:bg-indigo-200 p-1 rounded-md transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="flex items-center gap-3 w-full">
          ...
          <button
            ...
            className={cn(
              "p-3 border rounded-xl flex items-center justify-center transition-colors shadow-sm shrink-0",
              showFilters
        ...
        {showFilters && (
          <div className="flex flex-col gap-3 p-4 bg-white border border-slate-200 rounded-xl shadow-sm animate-in fade-in slide-in-from-top-2">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <button
                onClick={toggleAll}
                className="flex items-center gap-1.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded-lg text-xs font-semibold transition-colors"
              >
                {isAllExpanded ? <Minimize2 className="w-3.5 h-3.5" /> : <Maximize2 className="w-3.5 h-3.5" />}
                {isAllExpanded ? "Collapse All" : "Expand All Groups"}
              </button>
            </div>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-5">
              ...
              {/* Date filter */}
              <div className="flex flex-col gap-1.5 flex-[2] w-full min-w-[240px]">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full">
                  ...
                  {customRange && (
                    <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg flex-1 min-w-[200px]">
      ...
      {bulkMode && (
        <div className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-xl mb-4 animate-in slide-in-from-top-2 duration-200">
          <button
            onClick={() =>
              bulkSelected.size === filteredTransactions.length
                ? setBulkSelected(new Set())
                : selectAllFiltered()
            }
            className="flex items-center gap-1.5 text-xs font-bold hover:text-indigo-200"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs font-bold bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-lg disabled:opacity-40"
          >
          ...
          <button
            ...
            disabled={bulkSelected.size === 0}
            className="flex items-center gap-1 text-xs font-bold bg-red-500 hover:bg-red-600 px-3 py-1.5 rounded-lg disabled:opacity-40"
          >
            <Trash2 className="w-3.5 h-3.5" /> Delete
          </button>
          <button onClick={clearBulk} className="p-1.5 hover:bg-white/10 rounded-lg">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto space-y-6 pb-20">
        {groupedTransactions.length === 0 ? (
          ...
          groupedTransactions.map((group) => {
            ...
            const dateDisplay = isSameDay(parsedDate, today)
              ? "Today"
              : isSameDay(parsedDate, yesterday)
              ? "Yesterday"
                ...
                <div className="px-5 py-3 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={cn(
                        "w-5 h-5 rounded-md border-2 transition-all shrink-0",
                        bulkMode ? "border-indigo-500 bg-indigo-500" : "border-slate-300 hover:border-indigo-400"
                      )}
                  ...
                  {group.payeeGroups.map((pg) => {
                    ...
                    return (
                      <div key={groupId} className="flex flex-col">
                        ...
                        <div
                          onClick={() => {
                            if (bulkMode) { pg.transactions.forEach((tx) => toggleBulk(tx.id)); }
                            else if (isGrouped) { toggleGroup(groupId); }
                            else { setEditTxId(pg.transactions[0].id); }
                          }}
                          className={cn(
                        ...
                        >
                          <div className="flex items-center gap-4 flex-1 min-w-0">
                            ...
                            <div className={cn(
                              ...
                              pg.type === "income"  ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"
                            )}>
                                ...
                                {isGrouped ? (
                                  ...
                                  <>
                                    ...
                                    {pg.transactions[0].notes && (
                                      <><span className="shrink-0">•</span><span className="truncate">{pg.transactions[0].notes}</span></>
                                    )}
                                  </>
                                )}
                              </div>
                          ...
                          <div className="flex items-center gap-3 shrink-0 ml-2">
                            {!isGrouped && !bulkMode && (
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  onClick={(e) => { e.stopPropagation(); setEditTxId(pg.transactions[0].id); }}
                                ...
                                <button
                                  onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(pg.transactions[0].id); }}
                          ...
                          <div className="bg-slate-50/50 border-t border-slate-100 divide-y divide-slate-100 pl-16">
                            {pg.transactions.map((tx) => (
                              <div
                                key={tx.id}
                                className={cn(
                                  "p-3 pr-5 flex items-center justify-between text-sm transition-colors group/row cursor-pointer",
                                ...
                                <div className="flex items-center gap-3 shrink-0">
                                  {!bulkMode && (
                                    <div className="flex items-center gap-1 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                      <button onClick={(e) => { e.stopPropagation(); setEditTxId(tx.id); }} className="p-1 text-slate-400 hover:text-indigo-600 rounded-md hover:bg-indigo-50">
                                        <Edit className="w-4 h-4" />
                                      </button>
                                      <button onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(tx.id); }} className="p-1 text-slate-400 hover:text-red-600 rounded-md hover:bg-red-50">
                                        <Trash2 className="w-4 h-4" />
                                      </button>
                                    </div>
                                  )}
